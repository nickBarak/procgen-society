...
namespace Modules {
    public class GraticuleGenerator {
        public static int GraticuleSize;
        ...
        public static Tile[,] GenerateGraticule(List<Types.Classes.Abstract.AAgent> agents=null) {
            Tile[,] graticule = new Tile[GraticuleSize, GraticuleSize];
            for (s_latitude=0; s_latitude<GraticuleSize; s_latitude++) {
                for (s_longitude=0; s_longitude<GraticuleSize; s_longitude++) {
                    graticule[s_latitude, s_longitude] = new Tile(new int[2]{s_latitude, s_longitude});
                }
            }

            graticule = RoadGenerator.GenerateRoads(graticule);
            graticule = BuildingGenerator.GenerateBuildings(graticule);
            graticule = PeripheralTopographyGenerator.GeneratePeripheralTopography(graticule);
